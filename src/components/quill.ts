...
export const useQuillVideoHandlers = (quillRef: { current: { getEditor: () => any; }; }) => {
  // Handler para insertar video desde archivo
  const insertVideoFromFile = async (uploadEndpoint: string | URL, headers = {}) => {
    ...
    return new Promise((resolve, reject) => {
      input.onchange = async () => {
        ...
        try {
          ...
          const videoUrl = response.data.url || response.data.documentUrl;
          ...
          if (quill && videoUrl) {
            ...
            reject(new Error('No video URL received or Quill editor not found'));
          }
        } catch (error: any) {
          reject(new Error(`Video upload failed: ${error.response?.data?.message || error.message}`));
        }
      };
    });
  };
  ...
  const insertVideoFromUrl = (promptText = 'Ingresa la URL del video (YouTube, Vimeo, etc.):') => {
  ...
  const insertImageFromFile = async (uploadEndpoint: string | URL, headers = {}) => {
    const input = document.createElement('input');
    input.setAttribute('type', 'file');
      ...
      input.onchange = async () => {
        ...
        try {
          ...
          if (quill && imageUrl) {
            ...
            reject(new Error('No image URL received or Quill editor not found'));
          }
        } catch (error: any) {
          reject(new Error(`Image upload failed: ${error.response?.data?.message || error.message}`));
        }
      };
    });
  };
