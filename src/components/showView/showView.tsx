...
import { Title, Text, Image, Group, Badge, Stack, Card, SimpleGrid, Box, Divider } from '@mantine/core';
...
export const EntityFieldRenderer: React.FC<EntityFieldRendererProps> = ({
  ...
  const isImageUrl = (str: string): boolean => {
    ...
    const imageProviders = /imagekit\.io|cloudinary\.com|imgur\.com|firebasestorage\.googleapis\.com/i;
  ...
  const isHtml = (str: string): boolean => {
    ...
    return /<([a-zA-Z][\w:.-]*)(\s[^>]*)?>/i.test(trimmed) || /^<\/[a-zA-Z]/i.test(trimmed) || /^<!/i.test(trimmed);
  };
  ...
  const renderCoordinates = (coordinates: { latitude: number; longitude: number }) => {
    return (
  ...
  const renderComplexObject = (obj: any, fieldName: string): React.ReactNode => {
    if (isMongoObjectId(obj)) {
      return <Text c="dimmed" size="sm" ff="monospace">{obj.$oid}</Text>;
    }
    ...
    if (obj.address && obj.coordinates) {
      return (
        <Stack gap="xs">
          <Text><strong>Dirección:</strong> {obj.address}</Text>
          {renderCoordinates(obj.coordinates)}
        </Stack>
      );
    }

    const allBoolean = Object.values(obj).every(val => typeof val === 'boolean');
    if (allBoolean) {
      return (
    ...
    if (fieldName.toLowerCase().includes('style') || 
        Object.values(obj).some(val => typeof val === 'string' && isImageUrl(val))) {
      return (
        <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
          {Object.entries(obj).map(([key, value]) => (
            ...
            >
              <Text size="sm" fw={500} mb="xs">{formatFieldName(key)}</Text>
        ...
        <Stack gap="xs">
          {Object.entries(obj).map(([key, value]) => (
            <Group key={key} justify="space-between" wrap="nowrap">
              <Text size="sm" fw={500}>{formatFieldName(key)}:</Text>
      ...
      <Stack gap="sm">
        {arr.map((item, index) => (
          <Card key={index} withBorder p="sm" radius="sm">
            <Text size="sm" fw={500} mb="xs">Elemento {index + 1}</Text>
            <Divider mb="xs" />
            {typeof item === 'object' ? 
  ...
  // Claves ordenadas
  const dataKeys = Object.keys(data).filter(key => !excludeFields.includes(key));
